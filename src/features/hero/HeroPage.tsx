...
// Concentric RF "search" rings that radiate behind the hero character.
function RadarRings() {
  return (
    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
      {[0, 1, 2].map(i => (
        <span
          key={i}
          className="absolute rounded-full border-2 border-dora-orange/40 animate-radar"
          style={{ width: 260, height: 260, animationDelay: `${i}s` }}
        />
      ))}
      {/* Rotating sweep line */}
      <span
        className="absolute w-[130px] h-[2px] origin-left animate-sweep"
        style={{
          background: 'linear-gradient(90deg, rgba(255,140,66,0.55), transparent)',
        }}
      />
      <span className="absolute w-2.5 h-2.5 rounded-full bg-dora-orange animate-pulse-slow" />
    </div>
  )
}
...
export function HeroPage() {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-sky-canvas">
      ...
      {/* Hero composition: connect card + explorer character */}
      <div
        ...
        <div className="flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-16 w-full max-w-5xl">
          ...
          >
            ...
            <div className="rounded-[28px] border-2 border-[#FFD4A6] bg-cream-page shadow-dora-hero pt-8 pb-8 px-10">
              ...
              {/* Tagline */}
              <p className="text-center text-tale-gray font-body text-[13px] tracking-wide mb-7">
                RF Control &amp; Signal Explorer
              </p>
              ...
              <button
                ...
                className="w-full relative group py-4 px-6 rounded-full font-display font-bold text-lg text-white overflow-hidden transition-all duration-200 hover:-translate-y-0.5 disabled:cursor-default disabled:opacity-80 disabled:hover:translate-y-0"
                style={{
          ...
          {/* Explorer character — hidden on small screens to keep card focal */}
          <motion.div
            initial={{ opacity: 0, x: 60, scale: 0.9 }}
            animate={{ opacity: 1, x: 0, scale: 1 }}
            transition={{ duration: 0.7, ease: [0.34, 1.56, 0.64, 1], delay: 0.4 }}
            className="relative hidden lg:flex flex-col items-center justify-end w-[320px] h-[420px]"
          >
            <RadarRings />
            ...
            <div className="relative z-10 animate-float">
              <img
                ...
                className="w-[300px] drop-shadow-[0_18px_28px_rgba(45,42,62,0.22)]"
              />
            </div>

            {/* Ground shadow */}
            <div
              className="absolute bottom-2 left-1/2 -translate-x-1/2 w-44 h-5 rounded-full"
              style={{ background: 'radial-gradient(ellipse, rgba(45,42,62,0.18), transparent 70%)' }}
            />
            ...
            <img
              ...
              alt=""
              aria-hidden
              className="absolute bottom-1 -left-6 w-24 z-20 animate-float-slow drop-shadow-[0_10px_18px_rgba(45,42,62,0.20)]"
            />
          </motion.div>
        </div>
      </div>
